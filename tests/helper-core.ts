import { assertSnapshot } from 'lume/deps/snapshot.ts'
import lume from 'lume/mod.ts'
import { basename, fromFileUrl, join } from 'lume/deps/path.ts'
import { DeepPartial } from 'lume/core/utils/object.ts'

import type { default as Site, SiteOptions } from 'lume/core/site.ts'
import type { SourceMap } from 'lume/plugins/source_maps.ts'
...
function normalizeSourceMap(content: string) {
   ...
   sourceMap.sourceRoot = sourceMap.sourceRoot ? basename(sourceMap.sourceRoot) : undefined
   sourceMap.file = sourceMap.file ? basename(sourceMap.file) : undefined
   sourceMap.sources = sourceMap.sources.map((source: string) => basename(source))
